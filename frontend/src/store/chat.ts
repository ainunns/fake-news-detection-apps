...
export const useChat = create<UseChatProps>((set, get) => ({
    ...
    setChat: async (payload) =>
        set(({ chat }) => ({ chat: [...chat, payload] })),
    addChat: async (callback) => {
        const hasNewChat = get().chat.find(
